...
const Cart = ({
  ...
  if (Object.keys(cartItems).length > 0) {
    ...
    for (const key in cartItems) {
      cartItemsArr.push([key, cartItems[key]]);
    }
  }
  let sum = 0;
            ...
            return (
              <div className={styles.row} key={`${pId}_${pSize}`}>
                ...
                <div className={`row ${styles.textWrapper}`}>
                  <div
                    className={`col-md-5 col-sm-12 ${styles.cell} ${styles.pizza}`}
                  >
                    <h4>{product.title}</h4>
                    <span>
                      {`${sizes[pSize]} (${formatCurrency(
                        product.price2[pSize]
                      )} )`}
                    </span>
                  </div>
                  <div
                    className={`col-md-4 col-sm-12 ${styles.cell} ${styles.count}`}
                  >
                    <NumberInputGroup
                      id={`${pSize}${pId}`}
                  ...
                  <div
                    className={`col-md-3 col-sm-12 ${styles.cell} ${styles.price}`}
                  >
